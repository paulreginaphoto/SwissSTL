...
async def fetch_terrain_items(
    ...
    logger.info(f"Found {len(items)} terrain tiles for bbox {bbox_str} at {resolution}m (gsd={target_gsd})")
    return items
...
async def get_terrain_grid(
    ...
    items = await fetch_terrain_items(min_lon, min_lat, max_lon, max_lat, actual_resolution)
    ...
    for i, item in enumerate(items):
        path = await download_tile(item["href"])
        tile_paths.append(path)
        if on_progress:
            on_progress(5 + int((i + 1) / len(items) * 45))
